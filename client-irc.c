...
#endif
    // syslog(LOG_DEBUG, "send_irc_reply: last_arg = %p<%s>", last_arg,
    // last_arg);
  }
  va_end(ap);
  if (last_arg) {
...
static void send_irc_myinfo() {
  // send_irc_reply(IRC_RPL_MYINFO, "SSHOUT IRC frontend",
  // SSHOUT_VERSION_STRING, "wr", "n", NULL);
#ifdef IRC_SEND_EMPTY_PREFIX
  fputs(": ", stdout);
#endif
...
static void send_irc_luser(unsigned int user_count) {
  ...
  fputs(": ", stdout);
#endif
  printf(IRC_RPL_LUSERCLIENT
         " %s :There are %u users and 0 invisible on 1 servers\n",
...
static void send_irc_motd() {
  ...
  if (fd == -1) {
    ...
    if (e == ENOENT) {
      ...
      syslog(LOG_WARNING, "irc_command_motd: " SSHOUT_MOTD_FILE ": %s",
             strerror(e));
...
static void send_irc_user_join(const char *user_name) {
  if (strcmp(user_name, sshout_user_name) == 0)
    return;
...
/*static void irc_command_not_implemented(int fd, int argc, */
...
static void irc_command_nick(int fd, int argc,
                             struct fixed_length_string *argv) {
  ...
  if (argv->len != strlen(sshout_user_name) ||
      memcmp(argv->p, sshout_user_name, argv->len)) {
    send_irc_reply(IRC_ERR_RESTRICTED,
                   _("Nick name didn't match the registered one"), NULL);
    return;
  }
...
static void irc_command_user(int fd, int argc,
                             struct fixed_length_string *argv) {
  syslog(LOG_DEBUG, "function: irc_command_user(%d, %d, %p)", fd, argc, argv);
  if (argc < 4) {
    send_irc_reply(IRC_ERR_NEEDMOREPARAMS, "USER", _("Not enough parameters"),
                   NULL);
    return;
  }
  ...
  /*
          ...
          while(i <= argc) {
                  ...
                  if(argv[i].len) syslog(LOG_DEBUG, "argv[%d].p[0] = %hhu'%c'",
     i, argv[i].p[0], argv[i].p[0]); i++;
          }
  */
  if (is_irc_registered) {
    send_irc_reply(IRC_ERR_ALREADYREGISTRED, _("You cannot register again"),
                   NULL);
    return;
  }
  ...
  /*
          if(argv[1].len != 1) {
                  send_irc_reply(IRC_ERR_UNKNOWNMODE, "?", "User mode takes only
     1 numeric", NULL); } else if(argv[1].p[0] != '0') { char c[2] = {
     argv[1].p[0], 0 }; send_irc_reply(IRC_ERR_UNKNOWNMODE, c, "User modes other
     than 0 are not supported", NULL);
          }
  */
  if (is_irc_nick_name_set)
    do_registered(fd);
}

static void irc_command_oper(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc < 2) {
    send_irc_reply(IRC_ERR_NEEDMOREPARAMS, "OPER", _("Not enough parameters"),
                   NULL);
    return;
  }
...
static void irc_command_mode(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc < 1) {
    send_irc_reply(IRC_ERR_NEEDMOREPARAMS, "MODE", _("Not enough parameters"),
                   NULL);
    return;
  }
  ...
  if (argv->len != strlen(sshout_user_name) ||
      memcmp(argv->p, sshout_user_name, argv->len)) {
    send_irc_reply(IRC_ERR_USERSDONTMATCH,
                   _("You cannot change mode for other users"), NULL);
    return;
  }
}

static void irc_command_quit(int fd, int argc,
                             struct fixed_length_string *argv) {
...
static void irc_command_join(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc < 1) {
    send_irc_reply(IRC_ERR_NEEDMOREPARAMS, "JOIN", _("Not enough parameters"),
                   NULL);
    return;
  }
  ...
  channel[argv->len] = 0;
  send_irc_reply(IRC_ERR_NOSUCHCHANNEL, channel,
                 _("Only one channel #sshout is available"), NULL);
  return;
}
// is_irc_joined = 1;
strcpy(irc_channel_name, "#sshout");
...
static void irc_command_names(int fd, int argc,
                              struct fixed_length_string *argv) {
  // if(!*irc_channel_name) return;
  client_send_request_get_online_users(fd);
}

static void irc_command_list(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc > 1)
    return;
  if (argc == 1) {
...
static void irc_command_privmsg(int fd, int argc,
                                struct fixed_length_string *argv) {
  if (argc < 1) {
  ...
  if (!is_irc_registered) {
    send_irc_reply(IRC_ERR_NOTREGISTERED, "PRIVMSG",
                   _("You have not registered"), NULL);
    return;
  }
  struct local_message *message =
      malloc(sizeof(struct local_message) + argv[1].len);
  if (!message) {
...
static void irc_command_motd(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc > 0)
    return;
  send_irc_motd();
...
static void irc_command_version(int fd, int argc,
                                struct fixed_length_string *argv) {
  if (argc > 0)
    return;
  char buffer[506];
  ...
  snprintf(buffer, sizeof buffer,
           SSHOUT_VERSION_STRING "\n"
                                 "%s\n%s\n%s\n%s%s",
           _("IRC frontend"), SSHOUT_COPYRIGHT_LINE, SSHOUT_LICENSE_INFORMATION,
           _("Project page: "), " https://sourceforge.net/projects/sshout/");
#endif
  send_irc_reply(IRC_RPL_VERSION, buffer, NULL);
}

static void irc_command_time(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc > 0)
    return;
  time_t t = time(NULL);
  ...
  if (date_str_len)
    send_irc_reply(IRC_RPL_TIME, IRC_SERVER_NAME, buffer, NULL);
  else
    send_irc_reply(IRC_ERR_UNKNOWNERROR,
                   _("Error: cannot format current date time"));
}

static void irc_command_ping(int fd, int argc,
                             struct fixed_length_string *argv) {
  if (argc > 1)
    return;
#if 0
...
static void parse_irc_arg(const char *line, size_t len, int *argc,
                          struct fixed_length_string **argv) {
  ...
  do {
    ...
    if (line[i] == ':')
      do {
        ...
        (*argc)++;
        *argv =
            realloc(*argv, sizeof(struct fixed_length_string) * (*argc + 1));
        if (!*argv) {
...
static void do_irc_line(int fd, const char *line, size_t len) {
  ...
  if (space)
    parse_irc_arg(space + 1, len - command_len - 1, &argc, &argv);
  else
    memset(argv, 0, sizeof(struct fixed_length_string));
  struct command *c = irc_commands;
  while (c->name) {
    if (strlen(c->name) == command_len &&
        strncasecmp(c->name, line, command_len) == 0) {
      if (c->do_command)
        c->do_command(fd, argc, argv);
      free(argv);
      return;
    }
    c++;
...
static void client_irc_do_local_packet(int fd) {
  ...
  case GET_PACKET_ERROR:
    send_irc_reply(IRC_ERR_UNKNOWNERROR, _("Local packet read error"),
                   strerror(errno), NULL);
    close(fd);
    exit(1);
  ...
  case GET_PACKET_TOO_LARGE:
    send_irc_reply(IRC_ERR_UNKNOWNERROR, _("Received local packet too large"),
                   NULL);
    close(fd);
    exit(1);
  ...
  case SSHOUT_LOCAL_USER_NOT_FOUND:
    send_irc_reply(IRC_ERR_NOSUCHNICK, (char *)packet->data, "User not found",
                   NULL);
    break;
  default:
...
static void client_irc_do_stdin(int fd) {
  ...
  ss += s - skip_len;
  if (last_lf)
    memmove(input_buffer, last_lf + 1, ss);
