...
static enum key_types {
  ...
  switch (type_len) {
  case 3:
    if (strncasecmp(key, "RSA", 3) == 0)
      return KEY_RSA;
    if (strncasecmp(key, "DSA", 3) == 0)
      return KEY_DSA;
    return KEY_INVALID;
  case 5:
    if (strncasecmp(key, "ECDSA", 5) == 0)
      return KEY_ECDSA;
    return KEY_INVALID;
  case 7:
    if (strncasecmp(key, "ED25519", 7) == 0)
      return KEY_ED25519;
    if (memcmp(key, "ssh-rsa", 7) == 0)
      return KEY_RSA;
    if (memcmp(key, "ssh-dss", 7) == 0)
      return KEY_DSA;
    return KEY_INVALID;
  case 11:
    if (memcmp(key, "ssh-ed25519", 11) == 0)
      return KEY_ED25519;
    return KEY_INVALID;
  case 19:
    if (memcmp(key, "ecdsa-sha2-nistp", 16) == 0) {
      if (memcmp(key + 16, "256", 3) == 0 || memcmp(key + 16, "384", 3) == 0 ||
...
static int get_length_and_type_string_length_of_key_in_base64(
    const char *key, size_t *base64_len, size_t *type_len, char *buffer,
    size_t buffer_size) {
#if 0
...
#else
  *base64_len = 0;
  while (key[*base64_len] && key[*base64_len] != ' ')
    (*base64_len)++;
#endif
...
static int read_user_info(FILE *f, char **name, char **public_key,
                          char **comment, enum key_types *key_type,
                          size_t *line_len) {
  char line[4096];
  while (1) {
    nlines++;
    ...
    if (len == -2) {
      int c;
      fprintf(stderr,
              _("Warning: line %u in file " USER_LIST_FILE
    ...
    if (!q1) {
      fprintf(stderr,
              _("Warning: cannot find '\"' at line %u in file " USER_LIST_FILE
                "\n"),
              nlines);
      continue;
    }
    *q1 = 0;
    if (q1 - p < 8 || strcmp(q1 - 8, "command=") || strchr(p, ' ')) {
      fprintf(stderr,
              _("Warning: syntax error in file " USER_LIST_FILE " line %u\n"),
              nlines);
      continue;
    }
    q1++;
    ...
    if (!q2) {
      fprintf(stderr,
              _("Warning: unmatched '\"' in file " USER_LIST_FILE " line %u\n"),
              nlines);
      continue;
    }
    ...
    if (!user_name_len) {
      fprintf(
          stderr,
          _("Warning: empty user name in file " USER_LIST_FILE " line %u\n"),
          nlines);
      continue;
    }
    ...
    if (key_type_1 == KEY_INVALID) {
      *space = 0;
      fprintf(stderr,
              _("Warning: invalid key type '%s' in file " USER_LIST_FILE
                " line %u\n"),
              type_string, nlines);
      continue;
    }
    ...
    if (get_length_and_type_string_length_of_key_in_base64(
            base64, &base64_len, &inner_type_len, buffer, sizeof buffer) < 0)
      return 1;
    char *inner_key_type_string = buffer + 4;
    enum key_types key_type_2 =
        get_key_type(inner_key_type_string, inner_type_len);
    if (key_type_2 == KEY_INVALID) {
      inner_key_type_string[inner_type_len] = 0;
      fprintf(
          stderr,
          _("Warning: invalid key type '%s' from BASE64 in file " USER_LIST_FILE
            " line %u\n"),
    ...
    if (key_type_2 != key_type_1) {
      fprintf(stderr,
              _("Warning: key type didn't match in file " USER_LIST_FILE
                " line %u; key ignored\n"),
              nlines);
      continue;
    }
    *name = malloc(user_name_len + 1);
    if (!*name) {
      fprintf(stderr,
              _("Error: allocate %zu bytes failed when processing "
                "file " USER_LIST_FILE " line %u\n"),
    ...
    *public_key = malloc(type_len + 1 + base64_len + 1);
    if (!*public_key) {
      fprintf(stderr,
              _("Error: out of memory when processing file " USER_LIST_FILE
                " line %u\n"),
              nlines);
      return -1;
    }
...
static int ask_confirm() {
  ...
  do {
    ...
    if (len == -1 || strncasecmp(answer, "no", 2) == 0 ||
        strncmp(answer, "不", 3) == 0 || strcmp(answer, "否") == 0) {
      return 0;
    }
  } while (strncasecmp(answer, "yes", 3) && strncmp(answer, "是", 3) &&
           strncmp(answer, "好", 3) && strcmp(answer, "可以"));
  return 1;
}
...
static int adduser_command(int argc, char **argv) {
  ...
  if (space) {
    ...
    if (get_length_and_type_string_length_of_key_in_base64(
            key, &base64_len, &type_len, buffer, sizeof buffer) < 0)
      return 1;
  ...
  int existing_count = 0;
  {
    ...
    while (read_user_info(f, &user_name, &public_key, NULL, NULL, NULL) == 0) {
      if (strcmp(key, public_key) == 0) {
        free(key);
        fprintf(stderr,
                _("This public key is already used by user %s.\n"
  ...
  if (existing_count) {
    fprintf(stderr, _("%d key%s already exist for user %s\n"), existing_count,
            existing_count > 1 ? "s" : "", user);
    if (!force) {
      fprintf(stderr, _("Are you sure you want to add this key for user %s? "),
              user);
      if (!ask_confirm()) {
...
static int removeuser_command(int argc, char **argv) {
  ...
  char *user_name, *public_key;
  size_t line_len;
  while (read_user_info(f, &user_name, &public_key, NULL, NULL, &line_len) ==
         0) {
    if (strcmp(user, user_name) == 0) {
      if (line_count * sizeof(struct line_info) >= match_lines_allocated_size) {
        match_lines = realloc(match_lines, match_lines_allocated_size +=
                                           2 * sizeof(struct line_info));
        if (!match_lines) {
          perror("realloc");
          return 1;
        }
  ...
  if (line_count == 1) {
    ...
    if (!force) {
      fprintf(stderr,
              _("User %s have %zu public keys registered in the user list\n"),
              user, line_count);
      fprintf(stderr,
              _("If you want to remove only some of the user's keys, edit file "
                "'%s/" USER_LIST_FILE "' manually\n"),
              getenv("HOME"));
...
static int listuser_command(int argc, char **argv) {
  ...
  while (read_user_info(f, &user_name, &public_key, &comment, NULL, NULL) ==
         0) {
    ...
    if ((int)hash_type == -1) {
      ...
      printf(_("Public key fingerprint %s "),
             key_type_to_string(get_key_type(buffer + 4, type_len)));
      mhash(h, buffer, len);
      ...
      if (hash_type == MHASH_SHA256) {
        ...
        fputs(len < 0 ? _("Cannot encode SHA-256 fingerprint") : buffer,
              stdout);
      } else
        while (i < hash_len) {
          if (i)
...
int main(int argc, char **argv) {
  ...
  if (myeuid == 0) {
    ...
    fprintf(
        stderr,
        _("Current effective UID %u doesn't equal to sshout user account\n"),
        myeuid);
    return 1;
  }
  ...
  if (st.st_uid != pw->pw_uid) {
    fprintf(stderr,
            _("Home directory '%s' is not owned by sshout (expecting UID=%u, "
              "got %u)\n"),
