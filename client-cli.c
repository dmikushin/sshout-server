...
static void print_filtered(const char *text) {
  ...
  while (*text) {
    ...
    case '\x7f':
      if (use_color == -1)
        use_color = (option_color == COLOR_AUTO && isatty(STDOUT_FILENO)) ||
                    option_color == COLOR_ON;
      if (use_color)
        fputs(CSI_SGR_BRIGHT, stdout);
      printf("\\x%.2hhx", *text);
      if (use_color)
        fputs(CSI_SGR_RESET, stdout);
      break;
    default:
      putchar(*text);
      break;
...
static void write_preference(const char *name, const char *value) {
  if (!preference_file)
    return;
  if (fseek(preference_file, 0, SEEK_SET) < 0)
    return;
  ...
  while (1) {
    ...
    if (strncmp(buffer, name_equal, name_len + 1) == 0) {
      if ((size_t)len == name_len + 1 + value_len) {
        int n = fseek(preference_file, -1, SEEK_CUR) == 0 &&
                fgetc(preference_file) == '\n';
        if (fseek(preference_file, -n - value_len, SEEK_CUR) < 0)
          return;
        fputs(value, preference_file);
        fflush(preference_file);
        return;
      } else {
        if (fbackwardoverwrite(preference_file, len + 1) < 0)
          return;
        if (fseek(preference_file, 0, SEEK_END) < 0)
          return;
        break;
      }
    }
...
static void command_showhtml(int fd, int argc, char **argv) {
  ...
  if (strcmp(argv[1], "off") == 0)
    option_showhtml = SHOWHTML_OFF;
  else if (strcmp(argv[1], "color") == 0)
    option_showhtml = SHOWHTML_COLOR;
  else if (strcmp(argv[1], "plain") == 0)
    option_showhtml = SHOWHTML_PLAIN;
  else if (strcmp(argv[1], "raw") == 0)
    option_showhtml = SHOWHTML_RAW;
  else
    goto usage;
...
static void print_motd(int missing_ok) {
  ...
  if (fd == -1) {
    if (errno == ENOENT) {
      if (!missing_ok)
        print_with_time(-1, PRINT_NEWLINE, _("No MOTD available"));
      return;
    }
...
static void command_motd(int fd, int argc, char **argv) { print_motd(0); }
...
static void command_pasteimage(int fd, int argc, char **argv) {
  ...
  if (pid) {
    ...
    execlp("xclip", "xclip", "-o", "-selection", "clipboard", "-target",
           "image/jpeg", "-verbose", NULL);
    perror("xclip");
    _exit(127);
  }
}
...
static void print_message(const struct local_message *msg) {
  ...
  if ((option_color == COLOR_AUTO && isatty(STDOUT_FILENO)) ||
      option_color == COLOR_ON) {
    color_begin_from = strcmp(msg->msg_from, sshout_user_name) == 0
                           ? CSI_SGR_BRIGHT_GREEN
                           : CSI_SGR_BRIGHT_BLUE;
    color_begin_to = strcmp(msg->msg_to, sshout_user_name) == 0
                         ? CSI_SGR_BRIGHT_GREEN
                         : CSI_SGR_BRIGHT_BLUE;
  ...
  if (strcmp(msg->msg_to, GLOBAL_NAME) == 0 || strcmp(msg->msg_to, "*") == 0) {
    print_with_time(-1, 0, "%s%s%s: ", color_begin_from, msg->msg_from,
                    color_end);
  } else {
    print_with_time(-1, 0, "%s%s%s to %s%s%s: ", color_begin_from,
                    msg->msg_from, color_end, color_begin_to, msg->msg_to,
                    color_end);
  }
  if (need_parse_html) {
    putchar('\n');
...
static void client_cli_do_tick() {
  ...
  if (last_day == -1)
    ...
    if (date_str_len)
      print_with_time(t, PRINT_NEWLINE | PRINT_REDISPLAY_INPUT, "[%s]", buffer);
    else
      print_with_time(t, PRINT_NEWLINE | PRINT_REDISPLAY_INPUT,
                      _("Error: cannot format current date"));
    last_day = tm->tm_yday;
  }
}
...
static void open_preference(const char *user_name) {
  struct stat st;
  if (stat(SSHOUT_USERS_PREFERENCES_DIR, &st) < 0 &&
      (errno != ENOENT || mkdir(SSHOUT_USERS_PREFERENCES_DIR, 0750) < 0)) {
  ...
  memcpy(file_name, SSHOUT_USERS_PREFERENCES_DIR,
         sizeof SSHOUT_USERS_PREFERENCES_DIR - 1);
  file_name[sizeof SSHOUT_USERS_PREFERENCES_DIR - 1] = '/';
  memcpy(file_name + sizeof SSHOUT_USERS_PREFERENCES_DIR, user_name,
         user_name_len);
  ...
  while (1) {
    ...
    if (strncmp(buffer, "alert=", 6) == 0) {
      if (len == 7)
        option_alert = buffer[6] == '1';
      else if (fbackwardoverwrite(preference_file, len + 1) < 0)
        break;
    } else if (strncmp(buffer, "showhtml=", 9) == 0) {
      if (len == 10)
        option_showhtml = buffer[9] - '0';
      else if (fbackwardoverwrite(preference_file, len + 1) < 0)
        break;
    } else if (strncmp(buffer, "color=", 6) == 0) {
      if (len == 7)
        option_color = buffer[6] - '0';
      else if (fbackwardoverwrite(preference_file, len + 1) < 0)
        break;
    } else {
...
static void client_cli_init_io(const char *user_name) {
  if (client_log_only) {
    ...
    use_readline =
        (always_use_readline && *always_use_readline) || isatty(STDIN_FILENO);
  }
  if (use_readline) {
    rl_outstream = stderr;
    ...
    if (editing_mode) {
      if (strcmp(editing_mode, "emacs") == 0)
        rl_editing_mode = 1;
      else if (strcmp(editing_mode, "vi") == 0)
        rl_editing_mode = 0;
    }
  }
...
static void client_cli_do_local_packet(int fd) {
  ...
  case SSHOUT_LOCAL_USER_OFFLINE:
    print_with_time(-1, PRINT_NEWLINE | PRINT_REDISPLAY_INPUT,
                    ...
                    packet->type == SSHOUT_LOCAL_USER_ONLINE ? _("online")
                                                             : _("offline"));
    break;
