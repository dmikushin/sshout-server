...
static void broadcast_user_state(const char *user_name, int on,
...
static int user_online(int fd, const char *user_name, const char *host_name,
...
#ifdef HAVE_ICONV
  if (*text_encoding)
    user_text_encodings[fd] = strdup(text_encoding);
#endif
  syslog(LOG_INFO, "user %s login from %s fd %d dup %d", user_name, host_name,
         fd, found_dup);
#ifdef HAVE_UPDWTMPX
  if (access("wtmpx", W_OK) == 0) {
    ...
    if (gettimeofday(&tv, NULL) == 0) {
      ...
      struct utmpx utx = {.ut_type = USER_PROCESS,
                          .ut_pid = pid == -1 ? fd : pid,
...
#endif
  if (!found_dup)
    broadcast_user_state(user_name, 1, client_fds, fd_max_size);
  return 0;
}
...
static void user_offline(int fd, const fd_set *client_fds, int fd_max_size) {
  ...
  int i = 0;
  while (i < fd_max_size) {
    if (i != fd && FD_ISSET(i, client_fds) &&
        strcmp(user_names[i], user_name) == 0) {
      found_dup = 1;
      break;
    }
    i++;
  ...
  if (!found_dup)
    broadcast_user_state(user_name, 0, client_fds, fd_max_size);
...
static int send_online_users(int receiver_fd, int fd_max_size) {
  ...
  size_t packet_length = sizeof(struct local_packet) +
                         sizeof(struct local_online_users_info) +
                         sizeof(struct local_online_user) * count;
  ...
  struct local_online_users_info *info =
      (struct local_online_users_info *)packet->data;
...
#ifdef HAVE_ICONV
static int convert_message(const char *from_encoding, const char *to_encoding,
                           const char *sender_name,
                           const struct local_message *msg,
  ...
  *new_msg = malloc(sizeof(struct local_message) + msg->msg_length);
  ...
  size_t len;
  while ((len = iconv(cd, (char **)&input_p, &input_len, &output_p,
                      &output_len)) == (size_t)-1) {
    int e = errno;
    if (e != E2BIG) {
      iconv_close(cd);
    ...
    struct local_message *p = realloc(*new_msg, sizeof(struct local_message) +
                                                    (*new_msg)->msg_length);
    if (!p) {
      e = errno;
      iconv_close(cd);
...
static int dispatch_message(int sender_fd, const struct local_message *msg,
                            const fd_set *client_fds, int fd_max_size) {
  ...
  size_t packet_len = sizeof(struct local_packet) +
                      sizeof(struct local_message) + msg->msg_length;
  struct local_packet *packet = malloc(packet_len);
  if (!packet) {
  ...
  do {
    ...
    if (!is_broadcast && strcmp(user_names[fd], msg->msg_to)) {
      // Not the target user, but we also need to send the message back to
      // sender
      if (strcmp(user_names[fd], sender_name))
        continue;
    } else
      found = 1;
    ...
    if (fd != sender_fd && from_encoding && msg->msg_type == SSHOUT_MSG_PLAIN) {
      ...
      if (to_encoding && strcmp(from_encoding, to_encoding)) {
        struct local_message *new_msg;
        if (convert_message(from_encoding, to_encoding, sender_name, msg,
                            &new_msg) < 0) {
          syslog_perror(
              "dispatch_message: failed to convert text encoding from %s to %s",
              from_encoding, to_encoding);
        } else {
          cur_packet_len = sizeof(struct local_packet) +
                           sizeof(struct local_message) + new_msg->msg_length;
          cur_packet = malloc(cur_packet_len);
          if (!cur_packet) {
            syslog(LOG_ERR, "dispatch_message: out of memory when converting "
                            "character set");
            free(new_msg);
            continue;
          }
          ...
          memcpy(cur_packet->data, new_msg,
                 sizeof(struct local_message) + new_msg->msg_length);
          free(new_msg);
        }
      }
    ...
    if (sync_write(fd, cur_packet, cur_packet_len) < 0) {
      syslog_perror("dispatch_message: from %d to %d: write", sender_fd, fd);
      r = -1;
    }
#ifdef HAVE_ICONV
...
int server_mode(const struct sockaddr_un *socket_addr) {
  ...
  if (bind(fd, (struct sockaddr *)socket_addr, sizeof(struct sockaddr_un)) <
      0) {
  ...
  while (1) {
    ...
    if (FD_ISSET(fd, &rfdset)) {
      ...
      if (cfd == -1) {
        ...
        if (setsockopt(cfd, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof timeout) <
            0)
          syslog_perror("setsockopt");
    ...
    for (cfd = 0; n && cfd < FD_SETSIZE; cfd++) {
      ...
      if (FD_ISSET(cfd, &rfdset)) {
        ...
        case GET_PACKET_TOO_LARGE:
          syslog(LOG_WARNING, "client fd %d packet too large (%u bytes)", cfd,
                 (unsigned int)packet);
          goto end_of_connection;
        case GET_PACKET_OUT_OF_MEMORY:
          syslog(LOG_WARNING,
                 "client fd %d out of memory (allocating %u bytes)", cfd,
                 (unsigned int)packet);
          goto end_of_connection;
        case GET_PACKET_INCOMPLETE:
          // syslog(LOG_DEBUG, "client fd %d incomplete packet received, read
          // %zu bytes, total %zu bytes; will continue later", 	cfd,
          //buffers[cfd].read_length, buffers[cfd].total_length);
          continue;
        case 0:
          break;
        default:
...
#ifdef HAVE_ICONV
                      packet->data + USER_NAME_MAX_LENGTH +
        ...
        case SSHOUT_LOCAL_POST_MESSAGE:
          if (!*user_names[cfd]) {
            syslog(LOG_NOTICE, "client fd %d posting message without login",
                   cfd);
            break;
          }
          dispatch_message(cfd, (struct local_message *)packet->data, &fdset,
                           max_fd + 1);
          break;
        case SSHOUT_LOCAL_GET_ONLINE_USERS:
          if (send_online_users(cfd, max_fd + 1) < 0) {
            //	syslog(LOG_NOTICE, "client fd %d send_online_users failed,
            //disconnecting", cfd); 	goto end_of_connection;
          }
          break;
        default:
          syslog(LOG_WARNING, "client fd %d unknown packet type %d", cfd,
                 packet->type);
          break;
        }
        free(packet);
